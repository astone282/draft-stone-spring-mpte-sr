...
## Junction Binding Segment

The concept of a Junction Binding Segment (J-BSID) is introduced to describe the signaling and forwarding behavior of a Junction node in an SR network:
...
- combined with one or more Junction Binding Segments across a segment routed topology domain to form an MPTE DAG tunnel.

Since a Junction Binding Segment may egress to multiple downstream nodes, the endpoint of the corresponding SR Policy can be set to the null value (0.0.0.0 for IPv4, :: for IPv6).
Therefore, a Junction Binding Segment is identified by its <headend, color> attribute.

The color value is mapped to the <MID, Version> tuple and is tracked by the controller.

It is RECOMMENDED that a globally consistent color value be used across all Junction Binding Segments that belong to a single DAG instance or that serve a common DAG intent.

The MPTE tunnel is used by one or many SR Policies instantiated on one or many ingress nodes.

A Junction Binding Segment may be used by multiple ingress SR Policies, provided that the subgraph it forwards over is fully shared by all SR Policies using it, including the set of their respective egress nodes.

The ingress SR Policy is responsible for initiating traffic steering into the DAG and is associated with a color value representing service intent. The J-BSID, on the other hand,
is a DAG forwarding construct implemented as an SR Policy with a BSID.

To support global optimization with make-before-break (MBB) operations across a set of ingress SR Policies, the color value used by Junction Binding Segments SHOULD differ from the color values of the ingress SR Policies
...
The controller is responsible for maintaining and tracking the association and semantic meaning of color values across all Junction Binding Segments that participate in the DAG.

Accordingly, an implementation SHOULD treat ingress SR Policies and Junction Binding Segments as decoupled constructs, each with their own versioning and lifecycle.

Since SR-based networks support specifying multiple egress interfaces using adjacency-SID sets and Node SIDs,
the Junction Binding Segment MAY include a SID list entry that identifies multiple outgoing interfaces. In addition to egress interfaces,
{{I-D.draft-kompella-teas-mpte}} describes signaling ingress interfaces. The use of a Junction Binding Segment omits the need for per-interface
...
uses the same, single Junction Binding Segment value which is a Binding Segment.
...
The controller signals the Junction Binding Segment(s) to all downstream nodes, starting with the penultimate egress hop node(s) and
working upwards toward the ingress nodes. The controller may perform deployments in parallel provided there is no interdependency of values between J-BSIDs,
and the controller does not update the ingress until all deployments are deemed successful.

If a Junction Binding Segment cannot be deployed to a node, the controller SHOULD re-attempt the deployment up to a configurably defined maximum number of retries.
...
Junction Binding Segments are deployed as a unicast SR Policy with a single Candidate Path using protocols such as PCEP, BGP, or NETCONF.
...
upstream Junction nodes about their Junction Binding Segments in their outgoing SID lists. The BSID value MAY be a constant value globally, if assigned by PCE/Controller,
...
include at least two segment identifiers: one SID for forwarding to the downstream Junction node and one for the Junction Binding Segment value (BSID) of the downstream node.
...
Appendix A provides an example topology and resulting Junction Binding Segments.
...
As mentioned, the DAG tunnel may be re-used by multiple ingress SR Policies. This mechanism is used to support achieving multiple ingress nodes originated from the network,
...
When a packet with the BSID assigned to the Junction Binding Segment is received at its Junction Node,
the node performs weighted non-equal cost flow-based forwarding across all egress SID lists associated with the Junction Binding Segment.
...
Junction Binding Segments only to nodes with more than one egress segment list. In other words, if a node in the DAG has only one egress interface,
it functions solely as a transit node for an upstream Junction Binding Segment and does not receive a Junction Binding Segment itself.
For example, in the topology shown in Appendix A, Nodes E and F are omitted from receiving a Junction Binding Segment as each has only 1 egress link within the DAG.
...
applied per egress SID List on each Junction Binding Segment. Since the top SID(s) in each SID List identify the path to the
next downstream Junction node, TI-LFA is applicable. Effectively, TI-LFA is used to protect traffic between Junction Binding Segments along a path
...
DAG beyond the immediate downstream Junction node as it only knows the next Junction Binding Segment.
A controller MAY be used to precompute backup SR Paths and signal these backup SID Lists to the upstream Junction Binding Segments.

## Hierarchy

The use of Junction Binding Segments to achieve a DAG can be used in hierarchical organization and sharing of DAGs between end-to-end tunnels.
...
ABR-1 and the Egress X node. ABR-1 Junction Binding Segment which ingresses the DAG has a Binding Segment attached, for
...
As described in the Operational section, all transit nodes in a DAG MAY be signaled with a Junction Binding Segment.
Alternatively depending on the topological graph and TE requirements, two Junction Binding Segments may be interconnected via an
...
Since a Junction Binding Segment is realized via an SR Policy, the controller leverages existing protocol mechanisms to update a Junction Binding Segment forwarding
instructions, as the Junction Binding Segment itself is represented as a candidate path within the SR Policy. When updating an existing candidate path,
...
with make-before-break considerations necessitates deploying new Junction Binding Segments. This, in turn, requires the instantiation
...
Once all Junction Binding Segments are deployed to all Junction nodes, the ingress node is updated with new SID lists referencing the Binding SIDs of the new Junction Binding Segments downstream.
...
stable DAG version and its Junction Binding Segments to avoid inconsistencies in forwarding behavior.
...
Bidirectional Forwarding Detection is RECOMMENDED on each IP link to quickly detect link failures in the underlying topology. Single-hop BFD {{RFC5881}} is
applicable to link liveness detection, while Seamless BFD (S-BFD) {{RFC7880}} {{RFC7881}} is applicable to the DAG tunnel as described in [](#dag-tunnel). When a link
goes down, any tunnel S-BFD session that traverses the link will also fail. The controller tracks IGP topology changes
...
by each Junction Binding Segment.
...
Therefore, it is RECOMMENDED that each egress SID list of each Junction Binding Segment run its own S-BFD session {{RFC7880}} {{RFC7881}} {{RFC9612}}, initiated by the Junction Node
on which the Junction Binding Segment is installed. {{I-D.draft-ietf-spring-bfd}} describes monitoring the individual segment lists of an SR Policy candidate path and is
applicable to the egress SID lists of a Junction Binding Segment.

When an egress SID list includes the Binding SID of a downstream Junction Binding Segment, the S-BFD packet is not terminated on that transit Junction Node, it is carried through the downstream Junction Binding Segment(s) and terminates on a DAG egress node. An egress SID list that reaches a DAG egress directly likewise terminates there.

Consequently, a session deterministically exercises the segments imposed by the originating Junction Node, which is the path to the next downstream Junction Node or to a DAG egress node.
Beyond a downstream Binding SID, the packet is subject to the weighted forwarding performed by that downstream Junction Binding Segment, so the remainder of the path traversed is
selected by hashing. Coverage of every egress SID list in the DAG is therefore achieved by the union of the sessions run by all Junction Nodes rather than by any individual session.

The reverse direction of a session is not carried over the DAG. Reflected packets follow the routed path back to the originating Junction Node.

On session failure:

- The affected SID list is marked inactive and removed from the weighted ECMP set.
- If a Junction Binding Segment has no active egress SID lists remaining, the Junction Binding Segment becomes inactive, which in turn fails any
  upstream S-BFD sessions that use it.

Because a session traverses the downstream portion of the DAG, a failure located several hops downstream may fail the session of an upstream Junction Node whose own egress SID list would
remain functional when the downstream Junction node detects the failure. Therefore, the upstream node may temporarily mark a SID list to the downstream junction node inactive until the downstream Junction Node detects the failure and marks its own affected SID List as inactive, thus causing a hash rebalance to an alternative SID list.

Due to the numerous S-BFD sessions, an egress node may receive many S-BFD packets, one per SID list, per Junction Node, per DAG.

## Verifying Correct Operation

This section discusses OAM for SR-MPLS ({{RFC8287}}). Applicability to SRv6 ({{RFC9259}}) is to be covered in a future revision.

As with S-BFD described above, a single OAM probe from the ingress follows only one hashed path through the DAG and does not exercise every egress SID list on every Junction Node.

A controller can collect ping and traceroute results on a per-Junction, per-SID-list basis and aggregate them into a holistic view of the DAG. Each probe is sent over the path encoded in the SID list being tested, so that the forwarding provided by that SID list is verified independently of upstream hashing. As with S-BFD, when the tested SID list includes the Binding SID of a downstream Junction Binding Segment, the probe is not terminated on that Junction Node and is carried through the downstream Junction Binding Segment(s) to a DAG egress node. To elicit a response from the next downstream Junction Node itself, either the trailing Binding SID is omitted from the tested SID list, or TTL expiry based traceroute procedures are used. The controller may request results on-demand or receive them periodically. The frequency in which to run OAM is deployment specific.
...
The below Junction Binding Segments are deployed to the network realized with an SR Policy with a single Candidate Path containing multiple segment lists. Note the following:
...
- The path along {B,E,H} can be represented by a single Node SID. A Junction on node E is not required. Junction Binding Segment B may also be omitted (as described in [](#protection)) but is utilized here for example purposes.

- Node F also has a single egress link within the DAG, since {F,G} is pruned, therefore a Junction Binding Segment on node F is not required. Node F acts as a transit node for the
egress SID lists of Junction Binding Segments C and D, each of which imposes the adjacency SID from F to H directly.
...
- The color on the ingress SR Policy still provides intent steering for ingress traffic, therefore SHOULD be unique to the color value of the Junction Binding Segments
comprising the DAG to permit global make-before-break behavior.

~~~

Junction Binding Segment B:
  Color: 100
  BSID: BSID-B
  SID List 1: [Node-SID-H]

Junction Binding Segment G:
  Color: 100
  BSID: BSID-G
  SID List 1: [Adj-SID-GH]

Junction Binding Segment C:
  Color: 100
  BSID: BSID-C
  SID List 1: [Adj-SID-CB, BSID-B]
  SID List 2: [Adj-SID-CF, Adj-SID-FH]
  SID List 3: [Adj-SID-CG, BSID-G]
  SID List 4: [Node-SID-D, BSID-D]

Junction Binding Segment D:
  Color: 100
  BSID: BSID-D
  SID List 1: [Adj-SID-DF, Adj-SID-FH]
  SID List 2: [Adj-SID-DG, BSID-G]

Then lastly, at ingress the SR Policy transport tunnel is configured with the following:

Ingress SR Policy (Using Junction Binding Segments):
  Color: 50
  Candidate Path 1:
...
Ingress SR Policy (Ingress only):
  ...
  Candidate Path 1:
    ...
    SID List 4: [Adj-SID-AC, Adj-SID-CG, Adj-SID-GH]
    SID List 5: [Adj-SID-AC, Adj-SID-CB, Adj-SID-BE, Adj-SID-EH]
...
The following examples use the topology and Junction Binding Segments from the worked example in Appendix A.

Example 1 – Simple SID list update

- The Junction Binding Segment B is updated. The SID list [Node-SID-H] is replaced with SID List [Adj-SID-BE, Adj-SID-EH]
...
- The Junction Binding Segment B is no longer used.
- A delete is sent to remove Junction Binding Segment B
...
**Step 1 - Deploy new Junction Binding Segments**
...
The controller assigns and tracks color 2001 for the new DAG. The following new Junction Binding Segments are deployed in the following order. The BSID values are either assigned by the controller,
or requested by the node depending on the protocol in use. Note that after this deployment node Y has two Junction Binding Segments installed on it, both of which are active.

~~~
CREATE Junction Binding Segment U2:
  Color: 2001
  BSID: BSID-U2
  SID List 1: [Adj-SID-UX, ADJ-SID-XW]
  SID List 2: [Adj-SID-UW]

CREATE Junction Binding Segment Y2:
  Color: 2001
  BSID: BSID-Y2
  SID List 1: [Adj-SID-YX, ADJ-SID-XW]
  SID List 2: [Adj-SID-YU, BSID-U2]

CREATE Junction Binding Segment V2:
  Color: 2001
  BSID: BSID-V2
...
**Step 3 - Delete the no longer used Junction Binding Segments**

~~~
DELETE Junction Binding Segment Y1:
  Color: 2000
  BSID: BSID-Y1

DELETE Junction Binding Segment X1:
  Color: 2000
  BSID: BSID-X1
~~~
...
Thanks to Matthew Bocci, Richard "Footer" Foote, and Martin Vigoureux for discussions on this document. Thanks to Reshad Rahman for review comments.
****
